...
function Oe(n, e) {
  ...
  const l = t.tests.flatMap((a) => a.clashes).filter((a) => a.elements.some((r) => !r.guid)).length;
...
function Re(n, e) {
  ...
  for (const l of t.content.querySelectorAll("table")) {
    ...
    const a = { id: `${e.id}:t${e.tests.length}`, name: p, clashes: [] }, r = Array.from(l.rows), i = r.find((f) => Array.from(f.cells).some((g) => g.matches(".item1Header")));
    if (!i) {
      e.warnings.push(`Не распознаны колонки проверки «${p}».`);
      continue;
    }
    const h = Array.from(i.cells).flatMap((f) => Array.from({ length: f.colSpan }, () => ({ name: N(f), side: f.matches(".item1Header") ? 1 : f.matches(".item2Header") ? 2 : 0 })));
    let u = "";
    for (const f of r.filter((g) => g.matches(".contentRow,.childRow,.childRowLast,.clashGroupRow"))) {
      const g = ve(`${a.id}:c${a.clashes.length}`), w = [{}, {}];
      let v = 0;
      for (const x of Array.from(f.cells)) {
        const z = h[v];
        if (v += x.colSpan, !z) continue;
        const S = N(x), C = we(z.name);
        z.side ? w[z.side - 1][z.name] = S : (g.properties[z.name] = S, /наименованиеконфликта|clashname/.test(C) ? g.name = S : /статус|status/.test(C) ? g.status = S : /расстояние|distance/.test(C) ? g.distance = S : /описание|description/.test(C) ? g.description = S : /датаобнаружения|datefound/.test(C) ? g.date = S : /точкаконфликта|clashpoint/.test(C) && (g.point = Ne(S)));
      ...
      f.matches(".contentRow") && (u = ""), g.group = u, g.elements = [oe(w[0]), oe(w[1])], g.name && a.clashes.push(g), f.matches(".childRowLast") && (u = "");
    }
    e.tests.push(a);
  }
}
function Ie(n, e) {
  ...
  for (const p of t.querySelectorAll("clashtest")) {
    ...
    for (const a of p.querySelectorAll("clashresult")) {
      const r = ve(`${l.id}:c${l.clashes.length}`);
      for (const f of ["name", "status", "distance"]) r[f] = a.getAttribute(f) || "";
      r.description = N(a.querySelector("description")), r.image = se(a.getAttribute("href") || ""), r.group = a.closest("clashgroup")?.getAttribute("name") || "";
      const i = a.querySelector("createddate date");
      r.date = i ? ["year", "month", "day"].map((f) => i.getAttribute(f) || "").join("-") : "";
      const h = a.querySelector("clashpoint pos3f");
      h && ["x", "y", "z"].every((f) => h.hasAttribute(f) && Number.isFinite(Number(h.getAttribute(f)))) && (r.point = ["x", "y", "z"].map((f) => Number(h.getAttribute(f)))), r.properties = { Name: r.name, Status: r.status, Distance: r.distance, Description: r.description, "Clash point": r.point?.join("; ") || "" };
...
function Te(n) {
  ...
  for (const l of e.tests) {
    ...
    for (const a of l.clashes) {
      ...
      for (const r of a.elements) if (!r || !["guid", "id", "source", "name"].every((i) => t(r[i])) || !r.properties || typeof r.properties != "object" || !Object.values(r.properties).every(t)) throw Error("Повреждены свойства объекта.");
    }
  }
...
function Be(n) {
  const e = b;
  let t = 0;
  const p = n.tests.flatMap((a) => a.clashes.map((r) => {
    t++;
    const i = Ye(r.reviewed, r.excluded), h = [t, r.name, a.name, i, r.status, r.group, r.note, r.point?.join(" "), ...r.elements.flatMap((u) => [u.name, u.id, u.guid, u.source, ...Object.values(u.properties)]), ...Object.values(r.properties)].join(" ").toLowerCase();
    return `<tr data-state="${e(i)}" data-test="${e(a.name)}" data-search="${e(h)}"><td>${t}</td><td><strong>${e(r.name || `Коллизия ${t}`)}</strong></td><td>${e(a.name)}</td><td><span class="state ${r.excluded ? "excluded" : r.reviewed ? "done" : "work"}">${e(i)}</span><small>${r.enabled ? "Знак показывается" : "Знак скрыт"}</small></td><td>${e(r.status || "—")}</td><td>${e(r.group || "—")}</td><td>${e(r.note || "—")}</td><td>${e(r.point?.map((u) => Number(u.toFixed(3))).join("; ") || "—")}<small>${e(r.distance)}</small></td><td>${Ze(n.images[r.image] || "", r.name)}</td><td>${r.elements.map((u, f) => `<div class="object"><b>Элемент ${f + 1}</b><span>${e(u.name || u.id || "—")}</span><small>${e(u.source || "—")}</small><code>${e(u.guid || "GUID отсутствует")}</code></div>`).join("")}</td></tr>`;
...
function Ue(n, e, t, p, l = 6) {
  const [a, r, i] = n, h = r - e * 0.12, u = i + e * 1.9, f = i + e * 5, g = e * 1.65, w = [];
  p && w.push({ type: "line", a: [a, r, i], b: [a, r, u + e * 0.08], color: t, width: l }), w.push({ type: "polyline", points: [[a - g - e * 0.16, r, u - e * 0.14], [a + g + e * 0.16, r, u - e * 0.14], [a, r, f + e * 0.2], [a - g - e * 0.16, r, u - e * 0.14]], color: "#111111", fillColor: "#111111", width: 4 }), w.push({ type: "polyline", points: [[a - g + e * 0.15, r, u + e * 0.13], [a + g - e * 0.15, r, u + e * 0.13], [a, r, f - e * 0.18], [a - g + e * 0.15, r, u + e * 0.13]], color: t, fillColor: t, width: 2 });
  for (let z = 1; z <= 12; z++) {
    const S = z / 13, C = u + e * 0.13 + (f - u - e * 0.31) * S, O = (g - e * 0.15) * (1 - S) * 0.93;
    w.push({ type: "line", a: [a - O, r - e * 0.04, C], b: [a + O, r - e * 0.04, C], color: t, width: 12 });
  }
  const v = e * 0.13;
  w.push({ type: "polyline", points: [[a - v, h, i + e * 2.93], [a + v, h, i + e * 2.93], [a + v * 0.72, h, i + e * 4.13], [a - v * 0.72, h, i + e * 4.13], [a - v, h, i + e * 2.93]], color: "#ffffff", fillColor: "#ffffff", width: 2 });
  const x = e * 0.17;
  return w.push({ type: "polyline", points: [[a - x, h, i + e * 2.55], [a, h, i + e * 2.38], [a + x, h, i + e * 2.55], [a, h, i + e * 2.72], [a - x, h, i + e * 2.55]], color: "#ffffff", fillColor: "#ffffff", width: 2 }), w;
}
function qe(n, e, t, p, l, a, r) {
  const i = U(e, p), h = e.excluded ? "  [ИСКЛЮЧЕНА]" : e.reviewed ? "  [ОТРАБОТАНА]" : "", u = `#${t + 1}  ${e.name || "Коллизия"}${h}`, f = () => {
    l(e.id), ke(n, e, p);
  }, g = Ue(i, p.radius, a, p.showStem, p.stemWidth), w = [{ id: e.id, type: "shaped", shapes: g, activeShapes: g, activateCommand: f, dblCommand: f }];
  return r && w.push({ id: e.id + ":label", type: "simple", position: [i[0], i[1] - p.radius * 0.2, i[2] + p.radius * 5.35], attachment: "above", activateCommand: f, dblCommand: f, label: u, description: e.group || e.status || "Без статуса", labelColor: a === p.selectedColor ? "#171717" : "#ffffff", labelBackground: a }), w;
}
function he(n, e, t, p, l, a, r, i) {
  for (const h of qe(e, t, p, l, a, r, i)) n.add(h);
}
function Qe(n, e) {
...
function Ge(n, e, t, p, l = "") {
  ...
  let r = a.annotations.get(ie);
  r || (r = a.annotations.create(ie, 1e3));
  const i = a.annotations.get(ae);
  i && a.annotations.release(i);
  const h = a.annotations.create(ae, 1e4), u = Qe(e, t);
  ne.get(a)?.signature !== u && (r.clear(), e.forEach((x, z) => {
    if (!x.enabled || !U(x, t)) return;
    const S = x.excluded ? "#78818c" : x.reviewed ? t.reviewedColor : t.unreviewedColor;
    he(r, n, x, z, t, p, S, t.labelMode === "all");
  }));
  const f = e.findIndex((x) => x.id === l), g = e[f];
  g?.enabled && U(g, t) && he(h, n, g, f, t, p, t.selectedColor, t.labelMode === "selected"), r.visible = !0, h.visible = !0, ne.set(a, { signature: u }), a.invalidate();
...
function ke(n, e, t) {
  ...
  const a = t.navigationRadius, r = p, i = [1.35, -1.35, 0.8], h = Math.hypot(...i), u = [r[0] + i[0] / h * a, r[1] + i[1] / h * a, r[2] + i[2] / h * a], f = [r[0] - u[0], r[1] - u[1], r[2] - u[2]], g = Math.hypot(...f) || 1, w = [f[0] / g, f[1] / g, f[2] / g];
...
function K(n) {
  const e = [...We], t = new Set(e.map((l) => l.key)), p = (l, a, r) => {
    t.has(l) || (t.add(l), e.push({ key: l, label: a, source: r }));
  };
  for (const { clash: l } of n) {
    for (const a of Object.keys(l.properties || {})) p(`clash:${a}`, a, "HTML · коллизия");
    l.elements.forEach((a, r) => {
      for (const i of Object.keys(a.properties || {})) p(`e${r + 1}:prop:${i}`, i, `HTML · элемент ${r + 1}`);
    });
  }
  return e;
}
...
async function Je(n, e) {
  ...
  r.innerHTML = `<style>${Pe}${Xe}</style><main class="app">
    ...
    <dialog id="settings-dialog" class="columns-dialog settings-dialog"><div class="dialog-title"><div><small>ПАРАМЕТРЫ РАБОТЫ</small><h2>Настройки плагина</h2></div><button id="close-settings" aria-label="Закрыть">×</button></div>
      ...
      <section class="settings-page marker-settings-page" data-settings-page="camera" hidden><div class="settings-grid">
        ...
        ${["X", "Y", "Z"].map((o, s) => `<label>Сдвиг ${o}<input id="offset${o}" type="number" value="${l.offset[s]}" step="0.1"></label>`).join("")}
  ...
  const i = (o) => r.querySelector("#" + o);
  let h = [], u;
  const f = /* @__PURE__ */ new Set(), g = /* @__PURE__ */ new Set();
  let w = "", v = 0, x = !0;
  const z = 50, S = document.createElement("small");
  S.id = "save-state", S.setAttribute("role", "status"), r.querySelector("header").append(S);
  function C() {
    try {
  ...
  const q = (o, s = !1) => {
    i("message").textContent = o, i("message").classList.toggle("error", s);
  }, M = async (o) => {
    try {
      const s = await o();
      typeof s == "string" && q(s);
    } catch (s) {
      q(s instanceof Error ? s.message : String(s), !0);
    }
  }, I = () => {
  ...
  function E() {
    const o = i("search").value.toLowerCase(), s = i("status").value, d = i("review").value, c = i("visibility").value;
    return W().filter((m) => (!s || m.clash.status === s) && (!d || (d === "work" ? !m.clash.reviewed && !m.clash.excluded : d === "reviewed" ? m.clash.reviewed && !m.clash.excluded : m.clash.excluded)) && (!c || m.clash.enabled === (c === "yes")) && (!o || JSON.stringify(m).toLowerCase().includes(o)));
  }
  const le = () => $().find((o) => o.clash.id === w)?.clash;
  function Y(o, s = !0) {
    w = o;
    const d = E().findIndex((c) => c.clash.id === o);
    d >= 0 && (v = Math.floor(d / z)), j(), Z(), s && x && setTimeout(() => M(D), 0);
  }
  async function D() {
  ...
  function ze() {
    const o = K($()), s = new Map(a.order.map((d, c) => [d, c]));
    return o.sort((d, c) => (s.get(d.key) ?? 99999) - (s.get(c.key) ?? 99999)).filter((d) => (a.showAll || a.visible.includes(d.key)) && (!a.hideEmpty || $().some((c) => be(c, d.key).trim())));
  }
  const Se = (o) => a.names[o.key]?.trim() || o.label;
  function H() {
    const o = ee(W()), s = f.size ? `в наборах · ${f.size}` : "во всех проверках";
    i("summary").innerHTML = `<strong>${o.total}</strong><span>коллизий</span><div><b>${o.remaining}</b> надо отработать · ${o.reviewed} готово</div><small>${s}${o.excluded ? ` · ${o.excluded} исключено` : ""}${o.disabled ? ` · ${o.disabled} знаков скрыто` : ""}</small>`;
  }
  function Q() {
    const o = i("test-search").value.trim().toLowerCase(), s = u?.tests.filter((c) => !o || c.name.toLowerCase().includes(o)) || [];
    i("test-count").textContent = o ? `${s.length} / ${u?.tests.length || 0}` : String(u?.tests.length || 0);
    const d = ee($());
    i("tests").innerHTML = `<label class="test-item all ${f.size ? "" : "active"}"><input type="checkbox" data-all-tests ${f.size ? "" : "checked"}><span>Все проверки<small>${d.remaining} надо отработать</small></span><b>${d.total}</b></label>` + s.map((c) => {
      const m = ee(_(u, /* @__PURE__ */ new Set([c.id])));
      return `<label class="test-item ${f.has(c.id) ? "active" : ""}"><input type="checkbox" data-test="${b(c.id)}" ${f.has(c.id) ? "checked" : ""}><span>${b(c.name)}<small>${m.remaining} в работе${m.excluded ? ` · ${m.excluded} исключено` : ""}</small></span><b>${m.total}</b></label>`;
    }).join("");
  }
  function ce() {
  ...
  function j() {
    const o = E(), s = Math.ceil(o.length / z);
    v = Math.max(0, Math.min(v, Math.max(0, s - 1))), i("count").textContent = `Коллизии · ${o.length}`;
    const d = ze(), c = o.slice(v * z, v * z + z), m = c.length > 0 && c.every((y) => g.has(y.clash.id));
    i("list").innerHTML = c.length ? `<table class="data-grid"><thead><tr><th class="select-cell"><input id="select-page" type="checkbox" ${m ? "checked" : ""} aria-label="Выбрать строки страницы"></th>${d.map((y) => `<th title="${b(y.source)}">${b(Se(y))}</th>`).join("")}</tr></thead><tbody>${c.map((y) => `<tr data-clash="${b(y.clash.id)}" class="${y.clash.id === w ? "selected " : ""}${y.clash.excluded ? "excluded " : y.clash.reviewed ? "reviewed " : ""}${y.clash.enabled ? "" : "disabled"}"><td class="select-cell"><input class="row-check" type="checkbox" ${g.has(y.clash.id) ? "checked" : ""} aria-label="Добавить в массовый выбор"></td>${d.map((A) => {
      const L = be(y, A.key) || "—";
      return `<td title="${b(L)}">${b(L)}</td>`;
    }).join("")}</tr>`).join("")}</tbody></table>` : `<div class="empty welcome"><img src="${xe}" alt=""><h3>${u ? "Нет совпадений" : "Загрузите отчёт"}</h3><p>${u ? "Измените поиск или фильтры." : "Откройте HTML/XML Navisworks или ранее сохранённую сессию. После загрузки появятся таблица, наборы и знаки."}</p>${u ? "" : '<button class="primary" id="welcome-import">＋ Открыть отчёт</button>'}</div>`, i("page").textContent = o.length ? `${v + 1} / ${s}` : "0 / 0", i("prev-page").disabled = v === 0, i("next-page").disabled = (v + 1) * z >= o.length, ce();
  ...
  function P(o, s) {
    o.reviewed = s === "reviewed", o.excluded = s === "excluded", G();
  }
  function Ae(o, s) {
    const d = `<!doctype html><html lang="ru"><meta charset="utf-8"><title>${b(s)}</title><style>html,body{margin:0;width:100%;height:100%;background:#111;color:#fff}body{display:grid;place-items:center}img{max-width:100%;max-height:100%;object-fit:contain}</style><img src="${b(o)}" alt="${b(s)}">`, c = URL.createObjectURL(new Blob([d], { type: "text/html;charset=utf-8" })), m = window.open(c, "_blank", "width=1280,height=900");
    if (!m)
      throw URL.revokeObjectURL(c), Error("Браузер заблокировал окно снимка. Разрешите всплывающие окна для Топоматик 360.");
    m.opener = null, setTimeout(() => URL.revokeObjectURL(c), 6e4);
  }
  async function Me(o) {
    const s = E(), d = s.findIndex((m) => m.clash.id === w), c = s[d + o];
    if (c)
      return Y(c.clash.id, !1), await D(), e.focus(c.clash, T());
  }
  function pe() {
    const o = i("status"), s = o.value, d = [...new Set($().map((c) => c.clash.status))].filter(Boolean);
    o.innerHTML = '<option value="">Все статусы</option>' + d.map((c) => `<option value="${b(c)}">${b(c)}</option>`).join(""), o.value = d.includes(s) ? s : "";
  }
  function X() {
    i("reports").innerHTML = h.length ? h.map((o) => `<option value="${b(o.id)}">${b(o.name)}</option>`).join("") : '<option value="">Нет загруженных отчётов</option>', i("reports").value = u?.id || "", H(), Q(), pe(), j(), Z(), de();
  }
  function B() {
    const o = K($()), s = new Map(a.order.map((c, m) => [c, m])), d = o.sort((c, m) => (s.get(c.key) ?? 99999) - (s.get(m.key) ?? 99999));
    a.order = d.map((c) => c.key), i("column-settings").innerHTML = d.map((c, m) => `<tr data-column-row="${b(c.key)}"><td><input class="column-visible" type="checkbox" ${a.visible.includes(c.key) ? "checked" : ""}></td><td><input class="column-name" value="${b(a.names[c.key] || c.label)}"></td><td>${b(c.source)}</td><td><button class="column-up" ${m === 0 ? "disabled" : ""}>↑</button><button class="column-down" ${m === d.length - 1 ? "disabled" : ""}>↓</button></td></tr>`).join(""), i("show-all-columns").checked = a.showAll, i("hide-empty-columns").checked = a.hideEmpty;
  }
  function Ce(o, s) {
    const d = a.order.indexOf(o), c = d + s;
    d < 0 || c < 0 || c >= a.order.length || ([a.order[d], a.order[c]] = [a.order[c], a.order[d]], R(), B(), j());
  }
  function $e() {
    if (!u) throw Error("Сначала откройте отчёт.");
    const o = g.size ? g : new Set(W().map((s) => s.clash.id));
    return He(u, o);
  }
  function ue(o) {
    for (const s of $()) g.has(s.clash.id) && (s.clash.enabled = o);
    G();
  }
  function J(o) {
    for (const s of $()) g.has(s.clash.id) && (s.clash.reviewed = o === "reviewed", s.clash.excluded = o === "excluded");
    G();
  }
  async function De(o, s = !1) {
    const d = ye(o), c = h.find((m) => m.id === d.id);
    if (c && s)
      h = h.map((m) => m.id === d.id ? d : m), u = d;
    else if (c) {
      const m = new Map(c.tests.flatMap((k) => k.clashes).map((k) => [k.id, k]));
      ...
      d.images = { ...c.images, ...d.images }, h = h.map((k) => k.id === d.id ? d : k), u = d, q("Отчёт обновлён. Состояния и комментарии сохранены.");
    } else
      h.push(d), u = d;
    f.clear(), g.clear(), v = 0, w = $()[0]?.clash.id || "", x = !0, I(), X(), await D();
  }
  async function Ee(o) {
    let s = 0;
    for (const d of o.filter((c) => /\.(html?|xml|json)$/i.test(c.name))) {
      const c = await d.arrayBuffer();
      let m = new TextDecoder().decode(c);
      /charset\s*=\s*["']?windows-1251/i.test(m) && (m = new TextDecoder("windows-1251").decode(c));
      const k = /\.json$/i.test(d.name), y = k ? Te(m) : Oe(m, d.name);
      await me(y, o), await De(y, k), s++;
    }
    !s && u && (await me(u, o), I(), Z()), q(s ? `Загружено отчётов: ${s}. ${u?.warnings.join(" ") || ""}` : "Снимки подключены.");
  }
  async function me(o, s) {
    for (const d of new Set(o.tests.flatMap((c) => c.clashes.map((m) => m.image)).filter(Boolean))) {
      const c = s.filter((y) => {
        const A = se(y.webkitRelativePath || y.name);
        return A === d || A.endsWith("/" + d);
      }), m = s.filter((y) => y.name === d.split("/").at(-1)), k = c.length === 1 ? c[0] : m.length === 1 ? m[0] : void 0;
  ...
  for (const o of ["files", "session-file", "directory"]) i(o).onchange = () => M(async () => {
    const s = i(o);
    await Ee(Array.from(s.files || [])), s.value = "";
  });
  i("reports").onchange = () => M(async () => {
    u = h.find((o) => o.id === i("reports").value), f.clear(), g.clear(), w = $()[0]?.clash.id || "", v = 0, x = !0, X(), await D();
  }), i("test-search").oninput = () => Q(), i("tests").onchange = (o) => M(async () => {
    const s = o.target;
    if (s.matches("[data-all-tests]")) f.clear();
    else if (s.matches("[data-test]")) {
      const d = s.dataset.test;
      s.checked ? f.add(d) : f.delete(d);
    }
    v = 0, E().some((d) => d.clash.id === w) || (w = E()[0]?.clash.id || ""), H(), Q(), j(), Z(), await D();
  }), i("list").onclick = (o) => {
    const s = o.target;
    if (s.id === "select-page") {
      for (const c of E().slice(v * z, v * z + z)) s.checked ? g.add(c.clash.id) : g.delete(c.clash.id);
      j();
      return;
    }
    if (s.classList.contains("row-check")) {
      const c = s.closest("[data-clash]");
      c && (s.checked ? g.add(c.dataset.clash) : g.delete(c.dataset.clash)), ce();
      return;
    }
    if (window.getSelection()?.toString()) return;
    const d = s.closest("[data-clash]");
    d && Y(d.dataset.clash);
  }, i("list").ondblclick = (o) => {
    if (o.target.classList.contains("row-check")) return;
    const s = o.target.closest("[data-clash]");
    if (!s) return;
    Y(s.dataset.clash, !1);
    const d = le();
    d && M(async () => (await D(), e.focus(d, T())));
  };
  ...
  i("prev-page").onclick = () => {
    ...
    B(), i("settings-dialog").showModal();
  }, i("close-settings").onclick = () => i("settings-dialog").close(), i("settings-tabs").onclick = (o) => {
    const s = o.target.closest("[data-settings-tab]");
    if (s) {
      for (const d of r.querySelectorAll("[data-settings-tab]")) d.classList.toggle("active", d === s);
      for (const d of r.querySelectorAll("[data-settings-page]")) d.hidden = d.dataset.settingsPage !== s.dataset.settingsTab;
    }
  }, i("help").onclick = () => i("help-dialog").showModal(), i("close-help").onclick = () => i("help-dialog").close(), i("column-settings").onchange = (o) => {
    const s = o.target, d = s.closest("[data-column-row]");
    if (!d) return;
    const c = d.dataset.columnRow;
    s.classList.contains("column-visible") && (a.visible = s.checked ? [.../* @__PURE__ */ new Set([...a.visible, c])] : a.visible.filter((m) => m !== c)), s.classList.contains("column-name") && (a.names[c] = s.value), R(), j();
  }, i("column-settings").onclick = (o) => {
    const s = o.target.closest("button"), d = s?.closest("[data-column-row]");
    s && d && (o.preventDefault(), Ce(d.dataset.columnRow, s.classList.contains("column-up") ? -1 : 1));
    ...
    const o = $e();
    return fe(o.name.replace(/\.[^.]+$/, "") + "-review.html", Be(o), "text/html;charset=utf-8"), `HTML-отчёт сформирован: ${o.tests.reduce((s, d) => s + d.clashes.length, 0)} коллизий.`;
  }), X();
}
function te(n) {
