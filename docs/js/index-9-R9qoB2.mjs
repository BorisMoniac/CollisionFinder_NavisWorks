...
function Qe(n, e, t, p, l, a, s) {
  const i = U(e, p), g = e.excluded ? "  [ИСКЛЮЧЕНА]" : e.reviewed ? "  [ОТРАБОТАНА]" : "", u = `#${t + 1}  ${e.name || "Коллизия"}${g}`, f = () => {
    l(e.id), ke(n, e, p);
  }, h = Ue(i, p.radius, a, p.showStem, p.stemWidth), w = [{ id: e.id, type: "shaped", shapes: h, activeShapes: h, activateCommand: f, dblCommand: f }];
  return s && w.push({ id: e.id + ":label", type: "simple", position: [i[0], i[1] - p.radius * 0.2, i[2] + p.radius * 5.35], attachment: "above", activateCommand: f, dblCommand: f, label: u, description: e.group || e.status || "Без статуса", labelColor: a === p.selectedColor ? "#171717" : "#ffffff", labelBackground: a }), w;
}
function ge(n, e, t, p, l, a, s, i) {
  for (const g of Qe(e, t, p, l, a, s, i)) n.add(g);
}
function qe(n, e) {
...
function Ge(n, e, t, p, l = "") {
  ...
  ne.get(a)?.signature !== u && (s.clear(), e.forEach((x, z) => {
    ...
    ge(s, n, x, z, t, p, S, t.labelMode === "all");
  }));
  const f = e.findIndex((x) => x.id === l), h = e[f];
  h?.enabled && U(h, t) && ge(g, n, h, f, t, p, t.selectedColor, t.labelMode === "selected"), s.visible = !0, g.visible = !0, ne.set(a, { signature: u }), a.invalidate();
...
function ke(n, e, t) {
  ...
  const a = t.navigationRadius, s = [p[0], p[1], p[2] + t.radius * 3.45], i = [1.35, -1.35, 0.22], g = Math.hypot(...i), u = [s[0] + i[0] / g * a, s[1] + i[1] / g * a, s[2] + i[2] / g * a], f = [s[0] - u[0], s[1] - u[1], s[2] - u[2]], h = Math.hypot(...f) || 1, w = [f[0] / h, f[1] / h, f[2] / h];
  return l.camera?.id !== "3d" && l.setCameraType("3d"), l.lookAt(u, w, [0, 0, 1], !0, p), `Переход к ${e.name} с дистанцией ${a} м.`;
}
const He = [
...
function K(n) {
  const e = [...He], t = new Set(e.map((l) => l.key)), p = (l, a, s) => {
...
async function Je(n, e) {
  ...
  s.innerHTML = `<style>${Xe}${Pe}</style><main class="app">
    <header>
      <div class="brand"><img src="${xe}" alt=""><strong>НашеПО</strong><span class="version">0.5.3</span></div>
...
const Ve = {
  ...
  async mount(n) {
    ...
    t.style.height = "100%", e.replaceChildren(t), await Je(t, {
      mode: "Загрузите отчёт или откройте сохранённую сессию",
      markers: (p, l, a, s) => Ge(te(n), p, l, (i) => {
