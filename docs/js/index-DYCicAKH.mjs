const F = () => ({ guid: "", id: "", source: "", name: "", properties: {} }), we = (n) => n.toLowerCase().replace(/[^\p{L}\p{N}]/gu, "");
function Le(n) {
  let e = 2166136261;
  for (let t = 0; t < n.length; t++) e = Math.imul(e ^ n.charCodeAt(t), 16777619);
...
function re(n) {
  ...
  return n.trim().replace(/\\/g, "/").replace(/^\.\//, "");
}
function oe(n) {
  const e = Object.entries(n), t = (p) => e.find(([l]) => p.test(we(l)))?.[1] ?? "";
  return { guid: t(/ifcguid|globalid/), id: t(/^(объект)?id$|elementid|идентификаторэлемента/).replace(/^.*?:\s*/, ""), source: t(/файлиcточника|файлисточника|sourcefile/), name: t(/ifcname|^name$|^имя$/), properties: n };
}
const N = (n) => n?.textContent?.replace(/\s+/g, " ").trim() ?? "", ve = (n) => ({ id: n, name: "", status: "", distance: "", description: "", date: "", image: "", group: "", elements: [F(), F()], properties: {}, enabled: !0, reviewed: !1, excluded: !1, note: "" });
function Ne(n) {
  const e = ["X", "Y", "Z"].map((t) => n.match(new RegExp(t + "\\s*:\\s*([-+]?\\d+(?:[.,]\\d+)?(?:[eE][-+]?\\d+)?)", "i"))?.[1]);
  return e.every((t) => t !== void 0) ? e.map((t) => Number(t.replace(",", "."))) : void 0;
}
function Oe(n, e) {
  if (n.length > 100 * 1024 * 1024) throw Error("Отчёт больше 100 МБ. Разделите его на несколько проверок.");
  const t = { version: 1, id: Le(n), name: e, tests: [], images: {}, warnings: [] };
  if (/<\s*(?:\w+:)?(?:batchtest|clashtests|clashresults)\b/i.test(n) ? Ie(n, t) : Re(n, t), !t.tests.length) throw Error("Не найдены проверки Navisworks. Выберите отчёт HTML (табличный) или XML.");
  const l = t.tests.flatMap((a) => a.clashes).filter((a) => a.elements.some((s) => !s.guid)).length;
...
function Re(n, e) {
  ...
  for (const l of t.content.querySelectorAll("table")) {
    if (l.matches(".testSummaryTable")) {
      p = N(l.querySelector(".testName")) || p;
      continue;
    }
    if (!l.matches(".mainTable")) continue;
    const a = { id: `${e.id}:t${e.tests.length}`, name: p, clashes: [] }, s = Array.from(l.rows), i = s.find((f) => Array.from(f.cells).some((h) => h.matches(".item1Header")));
    if (!i) {
      e.warnings.push(`Не распознаны колонки проверки «${p}».`);
      continue;
    }
    const g = Array.from(i.cells).flatMap((f) => Array.from({ length: f.colSpan }, () => ({ name: N(f), side: f.matches(".item1Header") ? 1 : f.matches(".item2Header") ? 2 : 0 })));
    let u = "";
    for (const f of s.filter((h) => h.matches(".contentRow,.childRow,.childRowLast,.clashGroupRow"))) {
      const h = ve(`${a.id}:c${a.clashes.length}`), w = [{}, {}];
      let v = 0;
      for (const x of Array.from(f.cells)) {
        const z = g[v];
        if (v += x.colSpan, !z) continue;
        const S = N(x), C = we(z.name);
        z.side ? w[z.side - 1][z.name] = S : (h.properties[z.name] = S, /наименованиеконфликта|clashname/.test(C) ? h.name = S : /статус|status/.test(C) ? h.status = S : /расстояние|distance/.test(C) ? h.distance = S : /описание|description/.test(C) ? h.description = S : /датаобнаружения|datefound/.test(C) ? h.date = S : /точкаконфликта|clashpoint/.test(C) && (h.point = Ne(S)));
        const O = x.querySelector("img")?.getAttribute("src");
        O && (h.image = re(O));
      }
      if (f.matches(".clashGroupRow")) {
        u = h.name;
        continue;
      }
      f.matches(".contentRow") && (u = ""), h.group = u, h.elements = [oe(w[0]), oe(w[1])], h.name && a.clashes.push(h), f.matches(".childRowLast") && (u = "");
    }
    e.tests.push(a);
  }
}
function Ie(n, e) {
  if (/<!DOCTYPE|<!ENTITY/i.test(n)) throw Error("XML с DTD/ENTITY не поддерживается. Экспортируйте стандартный XML Navisworks.");
  ...
  for (const p of t.querySelectorAll("clashtest")) {
    const l = { id: `${e.id}:t${e.tests.length}`, name: p.getAttribute("name") || "Проверка", clashes: [] };
    for (const a of p.querySelectorAll("clashresult")) {
      const s = ve(`${l.id}:c${l.clashes.length}`);
      for (const f of ["name", "status", "distance"]) s[f] = a.getAttribute(f) || "";
      s.description = N(a.querySelector("description")), s.image = re(a.getAttribute("href") || ""), s.group = a.closest("clashgroup")?.getAttribute("name") || "";
      const i = a.querySelector("createddate date");
      s.date = i ? ["year", "month", "day"].map((f) => i.getAttribute(f) || "").join("-") : "";
      const g = a.querySelector("clashpoint pos3f");
      g && ["x", "y", "z"].every((f) => g.hasAttribute(f) && Number.isFinite(Number(g.getAttribute(f)))) && (s.point = ["x", "y", "z"].map((f) => Number(g.getAttribute(f)))), s.properties = { Name: s.name, Status: s.status, Distance: s.distance, Description: s.description, "Clash point": s.point?.join("; ") || "" };
      const u = Array.from(a.querySelectorAll("clashobjects clashobject")).map((f) => {
        const h = {};
        for (const x of f.querySelectorAll("objectattribute,smarttag")) {
          const z = N(x.querySelector("name"));
          z && (h[z] = N(x.querySelector("value")));
        }
        const w = oe(h), v = Array.from(f.querySelectorAll("pathlink node")).map(N);
        return w.source || (w.source = v.find((x) => /\.(ifc|smdx|rvt|nwc|nwd)$/i.test(x)) || ""), w.name || (w.name = N(f.querySelector("name")) || v.at(-1) || ""), w;
      });
      s.elements = [u[0] || F(), u[1] || F()], l.clashes.push(s);
    }
    e.tests.push(l);
  }
}
function ye(n) {
  for (const e of n.tests || []) for (const t of e.clashes || [])
...
function Te(n) {
  const e = JSON.parse(n), t = (l) => typeof l == "string";
  ...
  for (const l of e.tests) {
    ...
    for (const a of l.clashes) {
      if (!["id", "name", "status", "distance", "description", "date", "image", "group", "note"].every((s) => t(a[s])) || typeof a.reviewed != "boolean" || a.enabled !== void 0 && typeof a.enabled != "boolean" || a.excluded !== void 0 && typeof a.excluded != "boolean" || a.properties !== void 0 && (!a.properties || typeof a.properties != "object" || !Object.values(a.properties).every(t)) || !Array.isArray(a.elements) || a.elements.length !== 2 || p.has(a.id)) throw Error("Повреждена коллизия в сессии.");
      if (p.add(a.id), a.point !== void 0 && (!Array.isArray(a.point) || a.point.length !== 3 || !a.point.every(Number.isFinite))) throw Error("Неверные координаты.");
      for (const s of a.elements) if (!s || !["guid", "id", "source", "name"].every((i) => t(s[i])) || !s.properties || typeof s.properties != "object" || !Object.values(s.properties).every(t)) throw Error("Повреждены свойства объекта.");
    }
  }
...
const b = (n) => String(n ?? "").replace(/[&<>"']/g, (e) => ({ "&": "&amp;", "<": "&lt;", ">": "&gt;", '"': "&quot;", "'": "&#39;" })[e]), Ye = (n, e) => e ? "Исключена" : n ? "Отработана" : "В работе", Ze = (n, e) => n ? `<a href="${b(n)}" target="_blank" title="Открыть снимок"><img src="${b(n)}" alt="Снимок ${b(e)}"></a>` : "—";
function Be(n) {
  const e = b;
  let t = 0;
  const p = n.tests.flatMap((a) => a.clashes.map((s) => {
    t++;
    const i = Ye(s.reviewed, s.excluded), g = [t, s.name, a.name, i, s.status, s.group, s.note, s.point?.join(" "), ...s.elements.flatMap((u) => [u.name, u.id, u.guid, u.source, ...Object.values(u.properties)]), ...Object.values(s.properties)].join(" ").toLowerCase();
    return `<tr data-state="${e(i)}" data-test="${e(a.name)}" data-search="${e(g)}"><td>${t}</td><td><strong>${e(s.name || `Коллизия ${t}`)}</strong></td><td>${e(a.name)}</td><td><span class="state ${s.excluded ? "excluded" : s.reviewed ? "done" : "work"}">${e(i)}</span><small>${s.enabled ? "Знак показывается" : "Знак скрыт"}</small></td><td>${e(s.status || "—")}</td><td>${e(s.group || "—")}</td><td>${e(s.note || "—")}</td><td>${e(s.point?.map((u) => Number(u.toFixed(3))).join("; ") || "—")}<small>${e(s.distance)}</small></td><td>${Ze(n.images[s.image] || "", s.name)}</td><td>${s.elements.map((u, f) => `<div class="object"><b>Элемент ${f + 1}</b><span>${e(u.name || u.id || "—")}</span><small>${e(u.source || "—")}</small><code>${e(u.guid || "GUID отсутствует")}</code></div>`).join("")}</td></tr>`;
  })).join(""), l = n.tests.filter((a) => a.clashes.length).map((a) => `<option value="${e(a.name)}">${e(a.name)}</option>`).join("");
  return `<!doctype html><html lang="ru"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>${e(n.name)} — отчёт о коллизиях</title><style>
...
function fe(n, e, t) {
  const p = URL.createObjectURL(new Blob([e], { type: t })), l = document.createElement("a");
  l.href = p, l.download = n, l.click(), setTimeout(() => URL.revokeObjectURL(p), 3e4);
}
const he = () => ({ radius: 2, navigationRadius: 15, scale: 1, offset: [0, 0, 0], labelMode: "selected", showStem: !0, stemWidth: 6, unreviewedColor: "#e1372d", reviewedColor: "#28b94b", selectedColor: "#f2c94c" });
function U(n, e) {
  return n.point?.map((t, p) => t * e.scale + e.offset[p]);
}
function se(n) {
  if (!Number.isFinite(n.radius) || n.radius < 0.05 || !Number.isFinite(n.navigationRadius) || n.navigationRadius < 0.5 || !Number.isFinite(n.scale) || n.scale <= 0 || !n.offset.every(Number.isFinite)) throw Error("Размер знака и дистанция камеры должны быть положительными, смещения — конечными числами.");
  if (!Number.isFinite(n.stemWidth) || n.stemWidth < 1 || n.stemWidth > 30) throw Error("Толщина ножки должна быть от 1 до 30 пикселей.");
  if (!["selected", "all", "none"].includes(n.labelMode)) throw Error("Выбран неверный режим подписей.");
  if (![n.unreviewedColor, n.reviewedColor, n.selectedColor].every((e) => /^#[0-9a-f]{6}$/i.test(e))) throw Error("Цвет знака задан неверно.");
}
const ie = "nashepo.collision360.markers.v4", ae = "nashepo.collision360.selected.v4", ne = /* @__PURE__ */ new WeakMap();
function Ue(n, e, t, p, l = 6) {
  const [a, s, i] = n, g = s - e * 0.12, u = i + e * 1.9, f = i + e * 5, h = e * 1.65, w = [];
  p && w.push({ type: "line", a: [a, s, i], b: [a, s, u + e * 0.08], color: t, width: l }), w.push({ type: "polyline", points: [[a - h - e * 0.16, s, u - e * 0.14], [a + h + e * 0.16, s, u - e * 0.14], [a, s, f + e * 0.2], [a - h - e * 0.16, s, u - e * 0.14]], color: "#111111", fillColor: "#111111", width: 4 }), w.push({ type: "polyline", points: [[a - h + e * 0.15, s, u + e * 0.13], [a + h - e * 0.15, s, u + e * 0.13], [a, s, f - e * 0.18], [a - h + e * 0.15, s, u + e * 0.13]], color: t, fillColor: t, width: 2 });
  for (let z = 1; z <= 12; z++) {
    const S = z / 13, C = u + e * 0.13 + (f - u - e * 0.31) * S, O = (h - e * 0.15) * (1 - S) * 0.93;
    w.push({ type: "line", a: [a - O, s - e * 0.04, C], b: [a + O, s - e * 0.04, C], color: t, width: 12 });
  }
  const v = e * 0.13;
  w.push({ type: "polyline", points: [[a - v, g, i + e * 2.93], [a + v, g, i + e * 2.93], [a + v * 0.72, g, i + e * 4.13], [a - v * 0.72, g, i + e * 4.13], [a - v, g, i + e * 2.93]], color: "#ffffff", fillColor: "#ffffff", width: 2 });
  const x = e * 0.17;
  return w.push({ type: "polyline", points: [[a - x, g, i + e * 2.55], [a, g, i + e * 2.38], [a + x, g, i + e * 2.55], [a, g, i + e * 2.72], [a - x, g, i + e * 2.55]], color: "#ffffff", fillColor: "#ffffff", width: 2 }), w;
}
function ge(n, e, t, p, l, a, s) {
  const i = U(e, p), g = e.excluded ? "  [ИСКЛЮЧЕНА]" : e.reviewed ? "  [ОТРАБОТАНА]" : "", u = `#${t + 1}  ${e.name || "Коллизия"}${g}`, f = () => {
    l(e.id), ke(n, e, p);
  }, h = Ue(i, p.radius, a, p.showStem, p.stemWidth), w = { id: e.id, type: "simple", position: [i[0], i[1] - p.radius * 0.2, i[2] + p.radius * 5.35], shapes: h, activeShapes: h, attachment: "above", activateCommand: f, dblCommand: f };
  return s && Object.assign(w, { label: u, description: e.group || e.status || "Без статуса", labelColor: a === p.selectedColor ? "#171717" : "#ffffff", labelBackground: a }), w;
}
function Qe(n, e) {
  return JSON.stringify({ clashes: n.map((t) => [t.id, t.enabled, t.reviewed, t.excluded, t.name, t.group, t.status, t.point]), settings: { ...e, labelMode: e.labelMode === "all" ? "all" : "none" } });
}
function qe(n, e, t, p, l = "") {
  se(t);
  const a = n.cadview;
  if (!a) throw Error("Создайте или откройте проект Топоматик 360.");
  let s = a.annotations.get(ie);
  s || (s = a.annotations.create(ie, 1e3));
  const i = a.annotations.get(ae);
  i && a.annotations.release(i);
  const g = a.annotations.create(ae, 1e4), u = Qe(e, t);
  ne.get(a)?.signature !== u && (s.clear(), e.forEach((x, z) => {
    if (!x.enabled || !U(x, t)) return;
    const S = x.excluded ? "#78818c" : x.reviewed ? t.reviewedColor : t.unreviewedColor;
    s.add(ge(n, x, z, t, p, S, t.labelMode === "all"));
  }));
  const f = e.findIndex((x) => x.id === l), h = e[f];
  h?.enabled && U(h, t) && g.add(ge(n, h, f, t, p, t.selectedColor, t.labelMode === "selected")), s.visible = !0, g.visible = !0, ne.set(a, { signature: u }), a.invalidate();
  const w = e.filter((x) => x.enabled && U(x, t)).length, v = e.filter((x) => !x.enabled).length;
  return `Показано знаков: ${w}. Отключено: ${v}. Без координат: ${e.length - w - v}.`;
}
function Ge(n) {
  const e = n.cadview;
  if (e) {
    for (const t of [ie, ae]) {
      const p = e.annotations.get(t);
      p && e.annotations.release(p);
    }
...
function ke(n, e, t) {
  se(t);
  const p = U(e, t), l = n.cadview;
  if (!p) throw Error("В отчёте нет координат этой коллизии.");
  if (!l) throw Error("Откройте окно проекта Топоматик 360.");
  const a = t.navigationRadius, s = [1.35, -1.35, 0.8], i = Math.hypot(...s), g = [p[0] + s[0] / i * a, p[1] + s[1] / i * a, p[2] + s[2] / i * a], u = [p[0] - g[0], p[1] - g[1], p[2] - g[2]], f = Math.hypot(...u) || 1, h = [u[0] / f, u[1] / f, u[2] / f];
  return l.camera?.id !== "3d" && l.setCameraType("3d"), l.lookAt(g, h, [0, 0, 1], !0, p), `Переход к ${e.name} с дистанцией ${a} м.`;
}
const Fe = [
...
function K(n) {
  const e = [...Fe], t = new Set(e.map((l) => l.key)), p = (l, a, s) => {
    t.has(l) || (t.add(l), e.push({ key: l, label: a, source: s }));
  };
  for (const { clash: l } of n) {
    for (const a of Object.keys(l.properties || {})) p(`clash:${a}`, a, "HTML · коллизия");
    l.elements.forEach((a, s) => {
      for (const i of Object.keys(a.properties || {})) p(`e${s + 1}:prop:${i}`, i, `HTML · элемент ${s + 1}`);
    });
  }
  return e;
}
function be(n, e) {
  const t = n.clash, p = {
    ...
    "system:note": t.note,
    "system:point": t.point?.map((a) => Number(a.toFixed(3))).join("; ") || "",
...
function _(n, e) {
  let t = 0;
  return n?.tests.flatMap((p) => p.clashes.map((l) => ({ clash: l, test: p.name, testId: p.id, number: ++t }))).filter((p) => !e.size || e.has(p.testId)) || [];
}
function ee(n) {
  const e = n.map((t) => t.clash);
...
async function Pe(n, e) {
  ...
  s.innerHTML = `<style>${We}${Xe}</style><main class="app">
    <header>
      ...
      <div class="notice" role="status" id="message">${b(e.mode)}</div>
    </header>
    <div class="workspace">
      <aside>
        <div class="summary" id="summary"></div>
        <details class="settings"><summary>Знаки и камера</summary><div class="settings-grid">
          <label>Размер знака, м<input id="radius" type="number" value="${l.radius}" min="0.05" step="0.25"></label><label>Дистанция камеры, м<input id="navigation-radius" type="number" value="${l.navigationRadius}" min="0.5" step="1"></label>
          <label>Подписи<select id="label-mode"><option value="selected">Только выбранная</option><option value="all">Все</option><option value="none">Не показывать</option></select></label><label>Масштаб<select id="scale"><option value="1">1 — как в отчёте</option><option value="0.001">0,001 — мм → м</option><option value="1000">1000 — м → мм</option></select></label>
          ${["X", "Y", "Z"].map((o, r) => `<label>Сдвиг ${o}<input id="offset${o}" type="number" value="${l.offset[r]}" step="0.1"></label>`).join("")}
          <label>Цвет новых<input id="unreviewed-color" type="color" value="${b(l.unreviewedColor)}"></label><label>Цвет отработанных<input id="reviewed-color" type="color" value="${b(l.reviewedColor)}"></label><label>Цвет выбранного<input id="selected-color" type="color" value="${b(l.selectedColor)}"></label>
        ...
        </div><p>Щелчок по любой части знака выбирает коллизию и ставит центр координат в её точку.</p></details>
    ...
    <dialog id="columns-dialog" class="columns-dialog"><div class="dialog-title"><div><small>ТАБЛИЦА КОЛЛИЗИЙ</small><h2>Настройка столбцов HTML</h2></div><button id="close-columns">×</button></div><div class="column-options"><label class="check"><input id="show-all-columns" type="checkbox"> Все столбцы</label><label class="check"><input id="hide-empty-columns" type="checkbox"> Скрыть пустые</label><button id="select-columns">Выбрать все</button><button id="clear-columns">Снять все</button><button id="reset-columns">По умолчанию</button></div><div class="column-table-wrap"><table class="column-table"><thead><tr><th>Показывать</th><th>Имя столбца</th><th>Источник</th><th>Порядок</th></tr></thead><tbody id="column-settings"></tbody></table></div><p>Доступны все поля строки HTML и обоих элементов. Имя и порядок можно изменить.</p></dialog>
  ...
  function C() {
    try {
      ...
      return { ...he(), ...o, offset: Array.isArray(o.offset) ? o.offset : [0, 0, 0] };
    } catch {
      return he();
    }
  }
  function O() {
    try {
      return { visible: [...V], order: [], names: {}, showAll: !1, hideEmpty: !1, ...JSON.parse(localStorage.getItem(p) || "{}") };
    } catch {
  ...
  const Q = (o, r = !1) => {
    ...
    u && (S.textContent = "Есть несохранённые изменения");
  }, R = () => localStorage.setItem(p, JSON.stringify(a));
  function T() {
    return l.radius = Number(i("radius").value), l.navigationRadius = Number(i("navigation-radius").value), l.scale = Number(i("scale").value), l.labelMode = i("label-mode").value, l.offset = ["X", "Y", "Z"].map((o) => Number(i("offset" + o).value)), l.stemWidth = Number(i("stem-width").value), l.showStem = i("show-stem").checked, l.unreviewedColor = i("unreviewed-color").value, l.reviewedColor = i("reviewed-color").value, l.selectedColor = i("selected-color").value, se(l), localStorage.setItem(t, JSON.stringify(l)), l;
  }
  function $() {
    return _(u, /* @__PURE__ */ new Set());
  }
  function H() {
    return _(u, f);
  }
  function E() {
    const o = i("search").value.toLowerCase(), r = i("status").value, c = i("review").value, d = i("visibility").value;
    return H().filter((m) => (!r || m.clash.status === r) && (!c || (c === "work" ? !m.clash.reviewed && !m.clash.excluded : c === "reviewed" ? m.clash.reviewed && !m.clash.excluded : m.clash.excluded)) && (!d || m.clash.enabled === (d === "yes")) && (!o || JSON.stringify(m).toLowerCase().includes(o)));
  }
  const le = () => $().find((o) => o.clash.id === w)?.clash;
  function Y(o, r = !0) {
    w = o;
    const c = E().findIndex((d) => d.clash.id === o);
    c >= 0 && (v = Math.floor(c / z)), j(), Z(), r && x && setTimeout(() => M(D), 0);
  }
  async function D() {
    x && u && await e.markers(E().map((o) => o.clash), T(), Y, w);
  }
  function de() {
  ...
  function ze() {
    const o = K($()), r = new Map(a.order.map((c, d) => [c, d]));
    return o.sort((c, d) => (r.get(c.key) ?? 99999) - (r.get(d.key) ?? 99999)).filter((c) => (a.showAll || a.visible.includes(c.key)) && (!a.hideEmpty || $().some((d) => be(d, c.key).trim())));
  }
  const Se = (o) => a.names[o.key]?.trim() || o.label;
  function W() {
    const o = ee(H()), r = f.size ? `в наборах · ${f.size}` : "во всех проверках";
  ...
  function q() {
    const o = i("test-search").value.trim().toLowerCase(), r = u?.tests.filter((d) => !o || d.name.toLowerCase().includes(o)) || [];
    i("test-count").textContent = o ? `${r.length} / ${u?.tests.length || 0}` : String(u?.tests.length || 0);
    const c = ee($());
    i("tests").innerHTML = `<label class="test-item all ${f.size ? "" : "active"}"><input type="checkbox" data-all-tests ${f.size ? "" : "checked"}><span>Все проверки<small>${c.remaining} надо отработать</small></span><b>${c.total}</b></label>` + r.map((d) => {
      const m = ee(_(u, /* @__PURE__ */ new Set([d.id])));
      return `<label class="test-item ${f.has(d.id) ? "active" : ""}"><input type="checkbox" data-test="${b(d.id)}" ${f.has(d.id) ? "checked" : ""}><span>${b(d.name)}<small>${m.remaining} в работе${m.excluded ? ` · ${m.excluded} исключено` : ""}</small></span><b>${m.total}</b></label>`;
    }).join("");
  }
  function ce() {
    i("bulk").hidden = !h.size, i("bulk-count").textContent = `Выбрано: ${h.size}`;
  }
  function j() {
    const o = E(), r = Math.ceil(o.length / z);
    v = Math.max(0, Math.min(v, Math.max(0, r - 1))), i("count").textContent = `Коллизии · ${o.length}`;
    const c = ze(), d = o.slice(v * z, v * z + z), m = d.length > 0 && d.every((y) => h.has(y.clash.id));
    i("list").innerHTML = d.length ? `<table class="data-grid"><thead><tr><th class="select-cell"><input id="select-page" type="checkbox" ${m ? "checked" : ""} aria-label="Выбрать строки страницы"></th>${c.map((y) => `<th title="${b(y.source)}">${b(Se(y))}</th>`).join("")}</tr></thead><tbody>${d.map((y) => `<tr data-clash="${b(y.clash.id)}" class="${y.clash.id === w ? "selected " : ""}${y.clash.excluded ? "excluded " : y.clash.reviewed ? "reviewed " : ""}${y.clash.enabled ? "" : "disabled"}"><td class="select-cell"><input class="row-check" type="checkbox" ${h.has(y.clash.id) ? "checked" : ""} aria-label="Добавить в массовый выбор"></td>${c.map((A) => {
      const L = be(y, A.key) || "—";
      return `<td title="${b(L)}">${b(L)}</td>`;
    }).join("")}</tr>`).join("")}</tbody></table>` : `<div class="empty welcome"><img src="${xe}" alt=""><h3>${u ? "Нет совпадений" : "Загрузите отчёт"}</h3><p>${u ? "Измените поиск или фильтры." : "Откройте HTML/XML Navisworks или ранее сохранённую сессию. После загрузки появятся таблица, наборы и знаки."}</p>${u ? "" : '<button class="primary" id="welcome-import">＋ Открыть отчёт</button>'}</div>`, i("page").textContent = o.length ? `${v + 1} / ${r}` : "0 / 0", i("prev-page").disabled = v === 0, i("next-page").disabled = (v + 1) * z >= o.length, ce();
  ...
  function Z() {
    ...
    const r = u?.images[o.image], c = o.excluded ? "Исключена" : o.reviewed ? "Отработана" : "В работе";
    i("detail").innerHTML = `${r ? `<button class="snapshot snapshot-button" id="open-snapshot" title="Открыть снимок в отдельном окне"><img src="${b(r)}" alt="Снимок ${b(o.name)}"><span>Открыть крупнее</span></button>` : `<div class="snapshot"><div><b>Снимок не загружен</b><p>${b(o.image || "Изображение не указано")}</p></div></div>`}<div class="detail-controls"><div class="detail-actions"><button class="primary" id="focus">⌖ Перейти в 3D</button><button id="previous">← Предыдущая</button><button id="next">Следующая →</button></div><div class="coords">${o.point ? o.point.map((m, k) => `<div><small>${["X", "Y", "Z"][k]}</small><b>${m.toFixed(3)}</b></div>`).join("") : "В отчёте нет координат"}</div><div class="state-row"><label class="review-check" title="Включает или скрывает этот знак в 3D"><input id="enabled" type="checkbox" ${o.enabled ? "checked" : ""}> Показывать знак</label><strong class="work-state ${o.excluded ? "excluded" : o.reviewed ? "done" : "work"}">${c}</strong></div><div class="work-actions"><button id="work-review" class="${o.reviewed && !o.excluded ? "active" : ""}">✓ Отработана</button><button id="work-reopen" class="${!o.reviewed && !o.excluded ? "active" : ""}">Вернуть в работу</button><button id="work-exclude" class="${o.excluded ? "active danger" : ""}">Исключить</button></div><label title="Исходный статус из отчёта Navisworks">Статус Navisworks<input id="edit-status" value="${b(o.status)}" list="known-status"><datalist id="known-status">${["Новый", "Активн.", "Проверено", "Подтверждено", "Исправлено"].map((m) => `<option value="${m}">`).join("")}</datalist></label><label title="Исполнитель, раздел или пакет работ">Группа / назначение<input id="edit-group" value="${b(o.group)}" placeholder="Исполнитель или раздел"></label><label title="Ваш ответ по коллизии">Комментарий<textarea id="note" rows="3" placeholder="Решение, результат проверки…">${b(o.note)}</textarea></label></div><div class="detail-title"><small>КОЛЛИЗИЯ</small><h2>${b(o.name)}</h2><span>${b(o.description)}</span></div>${o.elements.map((m, k) => `<details class="element" ${k === 0 ? "open" : ""}><summary>Элемент ${k + 1} · ${b(m.name || m.id || "Без имени")}</summary><dl><dt>IFC GUID</dt><dd>${b(m.guid || "—")}</dd><dt>Источник</dt><dd>${b(m.source || "—")}</dd>${Object.entries(m.properties).map(([y, A]) => `<dt>${b(y)}</dt><dd>${b(A || "—")}</dd>`).join("")}</dl></details>`).join("")}`, i("focus").onclick = () => M(() => e.focus(o, T()));
    for (const [m, k] of [["previous", -1], ["next", 1]]) i(m).onclick = () => M(() => Me(k));
    i("enabled").onchange = () => je(o, i("enabled").checked), i("work-review").onclick = () => X(o, "reviewed"), i("work-reopen").onclick = () => X(o, "work"), i("work-exclude").onclick = () => X(o, "excluded");
    const d = i("open-snapshot");
    d && r && (d.onclick = () => Ae(r, o.name)), i("edit-status").onchange = () => {
      o.status = i("edit-status").value, I(), pe(), j();
    }, i("edit-group").oninput = () => {
      o.group = i("edit-group").value, I(), j();
    }, i("note").oninput = () => {
      o.note = i("note").value, I(), j();
    };
  }
  function G() {
    I(), W(), q(), j(), Z(), M(D);
  ...
  function Ae(o, r) {
    const c = `<!doctype html><html lang="ru"><meta charset="utf-8"><title>${b(r)}</title><style>html,body{margin:0;width:100%;height:100%;background:#111;color:#fff}body{display:grid;place-items:center}img{max-width:100%;max-height:100%;object-fit:contain}</style><img src="${b(o)}" alt="${b(r)}">`, d = URL.createObjectURL(new Blob([c], { type: "text/html;charset=utf-8" })), m = window.open(d, "_blank", "width=1280,height=900");
    if (!m)
      throw URL.revokeObjectURL(d), Error("Браузер заблокировал окно снимка. Разрешите всплывающие окна для Топоматик 360.");
    m.opener = null, setTimeout(() => URL.revokeObjectURL(d), 6e4);
  }
  async function Me(o) {
    const r = E(), c = r.findIndex((m) => m.clash.id === w), d = r[c + o];
    if (d)
      return Y(d.clash.id, !1), await D(), e.focus(d.clash, T());
  }
  function pe() {
    const o = i("status"), r = o.value, c = [...new Set($().map((d) => d.clash.status))].filter(Boolean);
    o.innerHTML = '<option value="">Все статусы</option>' + c.map((d) => `<option value="${b(d)}">${b(d)}</option>`).join(""), o.value = c.includes(r) ? r : "";
  }
  function P() {
    i("reports").innerHTML = g.length ? g.map((o) => `<option value="${b(o.id)}">${b(o.name)}</option>`).join("") : '<option value="">Нет загруженных отчётов</option>', i("reports").value = u?.id || "", W(), q(), pe(), j(), Z(), de();
  }
  function B() {
    const o = K($()), r = new Map(a.order.map((d, m) => [d, m])), c = o.sort((d, m) => (r.get(d.key) ?? 99999) - (r.get(m.key) ?? 99999));
    a.order = c.map((d) => d.key), i("column-settings").innerHTML = c.map((d, m) => `<tr data-column-row="${b(d.key)}"><td><input class="column-visible" type="checkbox" ${a.visible.includes(d.key) ? "checked" : ""}></td><td><input class="column-name" value="${b(a.names[d.key] || d.label)}"></td><td>${b(d.source)}</td><td><button class="column-up" ${m === 0 ? "disabled" : ""}>↑</button><button class="column-down" ${m === c.length - 1 ? "disabled" : ""}>↓</button></td></tr>`).join(""), i("show-all-columns").checked = a.showAll, i("hide-empty-columns").checked = a.hideEmpty;
  }
  function Ce(o, r) {
    const c = a.order.indexOf(o), d = c + r;
    c < 0 || d < 0 || d >= a.order.length || ([a.order[c], a.order[d]] = [a.order[d], a.order[c]], R(), B(), j());
  }
  function $e() {
    if (!u) throw Error("Сначала откройте отчёт.");
    const o = h.size ? h : new Set(H().map((r) => r.clash.id));
    return He(u, o);
  }
  function ue(o) {
    for (const r of $()) h.has(r.clash.id) && (r.clash.enabled = o);
    G();
  }
  function J(o) {
    for (const r of $()) h.has(r.clash.id) && (r.clash.reviewed = o === "reviewed", r.clash.excluded = o === "excluded");
    G();
  }
  async function De(o, r = !1) {
    const c = ye(o), d = g.find((m) => m.id === c.id);
    if (d && r)
      g = g.map((m) => m.id === c.id ? c : m), u = c;
    else if (d) {
      const m = new Map(d.tests.flatMap((k) => k.clashes).map((k) => [k.id, k]));
      for (const k of c.tests) for (const y of k.clashes) {
        const A = m.get(y.id);
        A && Object.assign(y, { enabled: A.enabled, reviewed: A.reviewed, excluded: A.excluded, note: A.note, group: A.group });
      }
      c.images = { ...d.images, ...c.images }, g = g.map((k) => k.id === c.id ? c : k), u = c, Q("Отчёт обновлён. Состояния и комментарии сохранены.");
    } else
      g.push(c), u = c;
    f.clear(), h.clear(), v = 0, w = $()[0]?.clash.id || "", x = !0, I(), P(), await D();
  }
  async function Ee(o) {
    let r = 0;
    for (const c of o.filter((d) => /\.(html?|xml|json)$/i.test(d.name))) {
      const d = await c.arrayBuffer();
      let m = new TextDecoder().decode(d);
      /charset\s*=\s*["']?windows-1251/i.test(m) && (m = new TextDecoder("windows-1251").decode(d));
      const k = /\.json$/i.test(c.name), y = k ? Te(m) : Oe(m, c.name);
      await me(y, o), await De(y, k), r++;
    }
    !r && u && (await me(u, o), I(), Z()), Q(r ? `Загружено отчётов: ${r}. ${u?.warnings.join(" ") || ""}` : "Снимки подключены.");
  }
  async function me(o, r) {
    for (const c of new Set(o.tests.flatMap((d) => d.clashes.map((m) => m.image)).filter(Boolean))) {
      const d = r.filter((y) => {
        const A = re(y.webkitRelativePath || y.name);
        return A === c || A.endsWith("/" + c);
      }), m = r.filter((y) => y.name === c.split("/").at(-1)), k = d.length === 1 ? d[0] : m.length === 1 ? m[0] : void 0;
      !k || !/^image\/(jpeg|png|webp)$/.test(k.type) || (o.images[c] = await new Promise((y, A) => {
  ...
  for (const o of ["radius", "navigation-radius", "stem-width", "scale", "label-mode", "offsetX", "offsetY", "offsetZ", "show-stem", "unreviewed-color", "reviewed-color", "selected-color"]) i(o).addEventListener("change", () => M(async () => (T(), await D(), x ? "Настройки применены." : "Настройки сохранены.")));
  ...
  for (const o of ["files", "session-file", "directory"]) i(o).onchange = () => M(async () => {
    const r = i(o);
    await Ee(Array.from(r.files || [])), r.value = "";
  });
  i("reports").onchange = () => M(async () => {
    u = g.find((o) => o.id === i("reports").value), f.clear(), h.clear(), w = $()[0]?.clash.id || "", v = 0, x = !0, P(), await D();
  }), i("test-search").oninput = () => q(), i("tests").onchange = (o) => M(async () => {
    const r = o.target;
    if (r.matches("[data-all-tests]")) f.clear();
    else if (r.matches("[data-test]")) {
      const c = r.dataset.test;
      r.checked ? f.add(c) : f.delete(c);
    }
    v = 0, E().some((c) => c.clash.id === w) || (w = E()[0]?.clash.id || ""), W(), q(), j(), Z(), await D();
  }), i("list").onclick = (o) => {
    const r = o.target;
    if (r.id === "select-page") {
      for (const d of E().slice(v * z, v * z + z)) r.checked ? h.add(d.clash.id) : h.delete(d.clash.id);
      j();
      return;
    }
    if (r.classList.contains("row-check")) {
      const d = r.closest("[data-clash]");
      d && (r.checked ? h.add(d.dataset.clash) : h.delete(d.dataset.clash)), ce();
      return;
    }
  ...
  i("prev-page").onclick = () => {
    ...
    v++, j();
  }, i("markers-toggle").onclick = () => M(async () => (x = !x, de(), x ? e.markers(E().map((o) => o.clash), T(), Y, w) : e.hide())), i("bulk-review").onclick = () => J("reviewed"), i("bulk-reopen").onclick = () => J("work"), i("bulk-exclude").onclick = () => J("excluded"), i("bulk-show").onclick = () => ue(!0), i("bulk-hide").onclick = () => ue(!1), i("clear-selection").onclick = () => {
    h.clear(), j();
  }, i("columns").onclick = () => {
    B(), i("columns-dialog").showModal();
  }, i("close-columns").onclick = () => i("columns-dialog").close(), i("help").onclick = () => i("help-dialog").showModal(), i("close-help").onclick = () => i("help-dialog").close(), i("column-settings").onchange = (o) => {
    const r = o.target, c = r.closest("[data-column-row]");
    if (!c) return;
    const d = c.dataset.columnRow;
    r.classList.contains("column-visible") && (a.visible = r.checked ? [.../* @__PURE__ */ new Set([...a.visible, d])] : a.visible.filter((m) => m !== d)), r.classList.contains("column-name") && (a.names[d] = r.value), R(), j();
  }, i("column-settings").onclick = (o) => {
    const r = o.target.closest("button"), c = r?.closest("[data-column-row]");
    r && c && (o.preventDefault(), Ce(c.dataset.columnRow, r.classList.contains("column-up") ? -1 : 1));
  }, i("show-all-columns").onchange = () => {
    a.showAll = i("show-all-columns").checked, R(), j();
  }, i("hide-empty-columns").onchange = () => {
    a.hideEmpty = i("hide-empty-columns").checked, R(), j();
  }, i("select-columns").onclick = () => {
    a.visible = K($()).map((o) => o.key), R(), B(), j();
  }, i("clear-columns").onclick = () => {
    a.visible = [], a.showAll = !1, R(), B(), j();
  }, i("reset-columns").onclick = () => {
    Object.assign(a, { visible: [...V], order: [], names: {}, showAll: !1, hideEmpty: !1 }), R(), B(), j();
  }, i("session").onclick = () => M(() => {
    if (!u) throw Error("Сначала откройте отчёт.");
    return fe(u.name.replace(/\.[^.]+$/, "") + ".collision360.json", JSON.stringify(u, null, 2), "application/json"), S.textContent = "Сессия сохранена", "Рабочая сессия сохранена. Для продолжения используйте «Открыть сессию».";
  }), i("export").onclick = () => M(() => {
    const o = $e();
    return fe(o.name.replace(/\.[^.]+$/, "") + "-review.html", Be(o), "text/html;charset=utf-8"), `HTML-отчёт сформирован: ${o.tests.reduce((r, c) => r + c.clashes.length, 0)} коллизий.`;
  }), P();
}
function te(n) {
  return new Proxy(n, { get(e, t) {
    return t === "app" ? n.manager.activeApp : t === "cadview" ? n.manager.activeWindow?.context : Reflect.get(e, t);
  } });
}
const Je = {
  open(n) {
    n.manager.revealView("nashepo.collision360/collision_panel");
  },
  async mount(n) {
    const e = n.el;
    if (!e) return;
    const t = document.createElement("div");
    t.style.height = "100%", e.replaceChildren(t), await Pe(t, {
      mode: "Загрузите отчёт или откройте сохранённую сессию",
      markers: (p, l, a, s) => qe(te(n), p, l, (i) => {
        n.manager.revealView("nashepo.collision360/collision_panel"), a(i);
      }, s),
      hide: () => Ge(te(n)),
      focus: (p, l) => ke(te(n), p, l)
    });
  }
};
export {
